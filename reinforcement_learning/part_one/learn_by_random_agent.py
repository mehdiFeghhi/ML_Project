...
def find_info_of_player(first_player,last_row_player_first,first_row_player_first,last_column_player_first,first_column_player_first):
...
def map_observation_two(observation):
    ...
    head_first_pos,head_first_affine_pos,position_head_first = find_info_of_player(first_player,last_row_player_first,first_row_player_first,last_column_player_first,first_column_player_first)

    # line_first = np.array([head_first_pos,head_first_affine_pos])

    head_second_pos,head_second_affine_pos,position_head_second = find_info_of_player(second_player,row_last_player_second,row_first_player_second,column_last_player_second,column_first_player_second)
    ...
    result = np.array([position_head_first,row_head_first-row_head_second,column_head_first-column_head_second,position_head_second])
    ...
    for _ in range(num_episodes):
        ...
        for agent in env.agent_iter():
            if agent == 'first_0':
                ...
                next_state = map_observation_two(observation)
...
def SARSA(env, lr=0.01, num_episodes=10000, eps=0.3, gamma=0.95, eps_decay=0.00005,file_read=None,number_epoch_run=0,number_action=18):
    # nA = number_action
    ...
    for ep in range(number_epoch_run,num_episodes):
        env.reset()
        observation, reward, termination, truncation, info = env.last()
        # arr_bytes = observation.tobytes()
        # state = hashlib.sha256(arr_bytes).hexdigest()
        # state = map_observation(observation)
        state = map_observation_two(observation)
        done = termination or truncation
        tot_rew = 0
        ...
        if (ep % 100) == 99:
            with open(f'checkpoint_main/v3_Q_Sarsa_learn_by_random_no_agent{ep}.pkl', 'wb') as f:
                pickle.dump(Q, f)
        if (ep % 300) == 299:
            print("I am in testing mood")

            with open('checkpoint_main/v3_Q_Sarsa_learn_by_random_agent.pkl', 'wb') as f:
                pickle.dump(Q, f)
            test_rew = run_episodes(env, Q, 10)

            print("Episode:{:5d}  Eps:{:2.4f}  Rew:{:2.4f}".format(ep, eps, test_rew))
            test_rewards.append(test_rew)
            with open('checkpoint_main/v3_test.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    Q_sarsa = SARSA(env, lr=.1, num_episodes=5000, eps=eps, gamma=0.95, eps_decay=0.001, file_read='checkpoint_main/v3_Q_Sarsa_learn_by_random_no_agent4999.pkl', number_epoch_run=4800)
    with open('checkpoint_main/v3_Q_Sarsa_learn_by_random_agent.pkl', 'wb') as f:
