...
class Chromosome:
    ...
    def grow(self, level=0):
        """
        Function to generate a tree in a growth manner
    ...
    def __make_preorder_to_post_order(self, poz=0):

        """
                Function to make preorder tree to
                @poz: current_position in genotype
                @return:
                """
        if self.gen[poz] in self.terminal_set:
            return self.gen[poz], poz
        elif self.gen[poz] in self.func_set[2]:
            poz_op = poz
            left, poz = self.__make_preorder_to_post_order(poz + 1)
            right, poz = self.__make_preorder_to_post_order(poz + 1)
            if self.gen[poz_op] == '+':
                return f'({left} + {right})', poz
            elif self.gen[poz_op] == '-':
                return f'({left} - {right})', poz
            elif self.gen[poz_op] == '*':
                return f'({left} * {right})', poz
            ...
            left, poz = self.__make_preorder_to_post_order(poz + 1)
            if self.gen[poz_op] == 'sin':
                return f'sin({left})', poz
            elif self.gen[poz_op] == 'cos':
                return f'cos({left})', poz
            elif self.gen[poz_op] == 'ln':
                return f'log({left})', poz
            elif self.gen[poz_op] == 'sqrt':
                return f'sqrt({left})', poz
            elif self.gen[poz_op] == 'tg':
                return f'tg({left})', poz
            elif self.gen[poz_op] == 'ctg':
                return f'ctg({left})', poz
            elif self.gen[poz_op] == 'e':
                return f'exp({left})', poz
            elif self.gen[poz_op] == 'tanh':
                return f'tanh({left})', poz
            elif self.gen[poz_op] == 'abs':
                return f'|{left}|', poz

    def __str__(self):

        return self.__make_preorder_to_post_order()[0]