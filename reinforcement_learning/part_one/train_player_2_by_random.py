...
def map_observation_two(observation,player=1):
    ...
    #
    # result = np.array([position_head_first, row_head_first - row_head_second, column_head_first - column_head_second,
    ...
    for _ in range(num_episodes):
        ...
        for agent in env.agent_iter():
            if agent == 'first_0':
                ...
                next_state = map_observation_two(observation,2)
...
def SARSA(env, lr=0.01, num_episodes=10000, eps=0.3, gamma=0.95, eps_decay=0.00005, file_read=None, number_epoch_run=0,
    ...
    for ep in range(number_epoch_run, num_episodes):
        ...
        state = map_observation_two(observation,2)
        done = termination or truncation
        tot_rew = 0
        ...
        for agent in env.agent_iter():
            ...
            if agent == 'first_0':
                ...
                env.step(rand_num)  # Take one step in the environment
                observation, reward, termination, truncation, info = env.last()
                done = termination or truncation
        ...
        if (ep % 100) == 99:
            with open(f'checkpoint_main/v3_Q_Sarsa_learn_by_random_agent{ep}_player_2.pkl', 'wb') as f:
                pickle.dump(Q, f)
        if (ep % 300) == 299:
            print("I am in testing mood")

            with open('checkpoint_main/v3_Q_Sarsa_learn_by_random_agent_player_2.pkl', 'wb') as f:
                pickle.dump(Q, f)
            test_rew = run_episodes(env, Q, 10)

            print("Episode:{:5d}  Eps:{:2.4f}  Rew:{:2.4f}".format(ep, eps, test_rew))
            test_rewards.append(test_rew)
            with open('checkpoint_main/v3_test_player_2.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    Q_sarsa = SARSA(env, lr=.1, num_episodes=5000, eps=0.01, gamma=0.95, eps_decay=0.001,
                    file_read='checkpoint_main/v3_Q_Sarsa_learn_by_random_agent4999_player_2.pkl', number_epoch_run=4501)
    with open('checkpoint_main/v3_Q_Sarsa_learn_by_random_agent_player_2.pkl', 'wb') as f:
