...
def Q_learning(env, lr=0.01, num_episodes=10000, eps=0.3, gamma=0.95, eps_decay=0.00005):
    ...
    for ep in range(num_episodes):
        ...
        # Test the policy every one episodes and print the results
        if (ep % 1) == 0:
            test_rew = run_episodes(env, Q, 20)
            print("Episode:{:5d}  Eps:{:2.4f}  Rew:{:2.4f}".format(ep, eps, test_rew))
            test_rewards.append(test_rew)

    with open(f'Q_part2.pkl', 'wb') as f:
...
def SARSA(env, lr=0.01, num_episodes=10000, eps=0.3, gamma=0.95, eps_decay=0.00005):
    ...
    with open(f'Q_Sarsa_part2.pkl', 'wb') as f:
        pickle.dump(test_rewards, f)
